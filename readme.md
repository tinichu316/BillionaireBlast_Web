...
## January 5, 2025: Alpha 1.0.0 release
...
* Holding `Shift` will cause you to move faster.
...
To accomplish the "server-wide" boss health, simple networking was implemented using a Firebase backend. This also served as a learning experience as this is my first game that implements any sort of networking. 
...
Beyond that, I want to expand the networking to eventually allow some sort of player progression. To that end, a login system with player accounts would be implemented, along with potentially some sort of currency or upgrade system. However, a simple approach for the time being would be to somehow increment a player's starting money by the sum of all the damage that they have dealt to the boss thus far.

There could also be a leaderboard that keeps track of the players who have dealt the most damage so far. I would also spend some more time developing the security rules and data validation on Firebase, as well as possibly implementing AppCheck reCAPTCHA.
